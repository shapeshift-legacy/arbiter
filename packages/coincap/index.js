...
let query_data = async function (coin,param) {
    ...
    try{
        ...
        if(debug1) console.log(tag,"rates: ",rates)
        ...
        for (var i = 0; i < map.length; i++) {
            if(debug1) console.log(tag,"map: ",map[i])
            if(map[i]){
                indexByName[map[i].name.toLowerCase()] = map[i].symbol.toLowerCase()
                indexBySymbol[map[i].symbol.toLowerCase()] = map[i].name.toLowerCase()
            }
        }
