...
LedgerBtc.prototype.signMessageNew_async = function(path, messageHex) {
	...
	return utils.foreach(apdus, function(apdu) {
		...
		return self.comm.exchange(buffer.toString('hex'), [0x9000]).then(function(apduResponse) {
				...
				console.log("hex sig: ",response.toString('base64'))
            	//let hash = Buffer.from(apduResponse, 'hex').toString('base64')
            ...
            let hash = encodeSignature(response, false, true)
				...
				//let hash = r.toString('base64')+s.toString('base64')
