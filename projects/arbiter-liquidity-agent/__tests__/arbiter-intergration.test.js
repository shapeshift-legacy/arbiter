...
describe(' - Intergration test for project Arbiter liquidty agent - ', () => {
    ...
    test('config is required correctly', () => {
        // get account info
        // log.debug(config)
        expect(process.env['AGENT_BTC_SIGNING_PRIVKEY']).toBeDefined()
        expect(process.env['AGENT_BTC_MASTER']).toBeDefined()


        expect(process.env['TEST_NET']).toEqual('true')
    })
    ...
    test.only('get orders', async () => {
        // get account info
        let payload = await arbiter.orders()
        log.debug('payload: ', payload)
    ...
    //test1
    let test1Tag = "limit order (BTC_LTC)"
    let test1FundingSat

    test('agent can place '+test1Tag, async () => {
        let tag = TAG+" | "+test1Tag+" | "
        ...
        //figure out amount of BTC 0.001 LTC costs
        let amountBTC = quantity * parseFloat(rate)
        log.debug(tag,"amountBTC: ",amountBTC)

        //find amount in satoshi
        let amountSat = amountBTC * 100000000
        amountSat = parseInt(amountSat)
        log.debug(tag,"amountSat: ",amountSat)
        test1FundingSat = amountSat
        ...
        let expectedBalance = balanceStart - amountSat
    ...
    test('agent can cancel limit order (BTC_LTC)', async () => {
        ...
        let expectedBalance = balanceStart + test1FundingSat
        expect(newBalance).toEqual(expectedBalance)
    })
    ...
    //ETH
    orderId = uuid.v4()
    let test2Tag = "limit order (BTC_ETH)"
    let test2FundingSat

    log.debug('new orderId: ', orderId)
    test('agent can place '+test2Tag, async () => {
        let tag = ' | '+test2Tag+' | '
        balanceStart = await arbiter.balance('BTC')
        log.debug(tag, 'balanceStart: ', balanceStart)
        balanceStart = parseFloat(balanceStart) * 100000000
        ...
        //figure out amount of BTC 0.001 ETH costs
        let amountBTC = quantity * parseFloat(rate)
        log.debug(tag,"amountBTC: ",amountBTC)
        ...
        log.debug(tag, 'balanceStart: ', balanceStart)
        log.debug(tag, 'balanceStart: ', typeof (balanceStart))
        let expectedBalance = balanceStart - test2FundingSat
        expect(newBalance).toEqual(expectedBalance)
    })
    ...
    orderId = uuid.v4()
    log.debug('new orderId: ', orderId)
    let test3Tag = "limit order (BTC_GNT)"
    let test3FundingSat
    ...
    test('agent can place limit order (BTC_GNT)', async () => {
        ...
        log.debug(tag, 'balanceStart: ', balanceStart)
        log.debug(tag, 'balanceStart: ', typeof (balanceStart))
        let expectedBalance = balanceStart - test3FundingSat
        expect(newBalance).toEqual(expectedBalance)
    })
