...
function Server(options){
    ...
    var server = this.agent.createServer(function(req, res){
        ...
        process.on('unhandledRejection', err => {
          console.error(`unexpected error during RPC request`, {
            data: JSON.stringify(data),
            err
          })

          let reply = {
            error: "unexpected error during RPC request: " + err.message
          }
        ...
        req.on('end', async function(){
            ...
            console.log(`rpc request: `, JSON.stringify(data, false, ' '))
            if(data && data.method && data.params){
                if(Object.keys(methods).indexOf(data.method)>-1){
                    var execMethod = methods[data.method];

                    try {
                      reply = await execMethod(data.params, data.id);
                    } catch (ex) {
                      console.error(`unexpected error during RPC request`, { data, ex })
                      reply = {
                        error: "unexpected error during RPC request: " + ex.message
                      }
                    }
                }
            }
    ...
    server.setTimeout(300000, () => { console.error(`server side timeout!`, arguments) })
