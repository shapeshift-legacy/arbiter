...
const getMarketsConfig = () => {
    try {
        let markets = (process.env.MARKETS || 'LTC_BTC,ETH_BTC').split(',')
        let pairs = new Set()
        let coins = new Set()
...
let config = {
    ...
    OAUTH_ROOT: process.env['OAUTH_ROOT'],
    ...
    FREE_MONEY_START:process.env['FREE_MONEY_START']  || 100000000,

    //Master addresses
    MASTER_LTC : process.env['MASTER_LTC']         || '',
    MASTER_BTC : process.env['MASTER_BTC']         || '',
    MASTER_ETH : process.env['MASTER_ETH']         || process.env['ARBITER_MASTER_ETH'],
    MASTER_GNT : process.env['MASTER_GNT']         || process.env['ARBITER_MASTER_ETH'],
