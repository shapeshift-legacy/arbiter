...
const send_utxo_transaction = async function (orderInfo, accountInfo, amount) {
    ...
    try {
        ...
        //TODO abstract this to aman!! fuuccckkk
        let txSigned
        // if(coin === "BTC"){
        //     txSigned = await daemons[coin.toLowerCase()].signRawTransactionWithWallet(resultTx)
        //     log.debug('txSigned: ', txSigned)
        // }else{
            txSigned = await daemons[coin.toLowerCase()].signRawTransaction(resultTx)
            log.debug('txSigned: ', txSigned)
        // }
