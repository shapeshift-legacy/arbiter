...
const cancel_order = async function (account, nonce, orderId) {
    ...
    try {
        ...
        output.event = 'cancel'
        output.txid = 'cancel:'+orderId
        output.time = new Date().getTime()
        publisher.publish("arbiterLa",JSON.stringify(output))
        try{
...
const account_info = async function (account) {
    ...
    try {
        ...
        log.debug(tag, balances)

        output.type = 'Liquidity agent'
        output.account = account
        if(!balances.nonce) balances.nonce = 0
...
const account_address = async function (account, coin) {
    ...
    try {
        ...
        if (!address) {
            ...
            let newAddress = await daemons[coin.toLowerCase()].getNewAddress()
            ...
            log.debug(tag, 'newAddress', newAddress)
            log.debug(tag, 'typeof addressInfo', typeof addressInfo)
...
const account_withdraw = async function (account, coin, address, amount) {
    ...
    try {
        ...
        // publish event
        output.newBalance = output.newBalance / SATOSHI
