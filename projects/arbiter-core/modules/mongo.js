...
const connection = monk(connectionString, options, err => {
    if (err) {
        log.error(TAG,`Error connecting to mongo!`, err)
        throw new Error(err)
    } else {
...
let ordersLA = connection.get('ordersLA')
let orders = connection.get('orders')
let users = connection.get('users')



module.exports = exports = {match,balances,credits,debits,orders,users,reportARB,reportLA,reportSignup,reportAsym,ordersLA}
