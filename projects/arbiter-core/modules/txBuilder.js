/**
 * Bitcoin Transaction Builder tools.
 *                          -Project Arbiter
 *
 *
 * Created by highlander on 2/28/17.
 */
const TAG = ' | txBuilder | '
...
module.exports = {
    ...
    // Build address
    buildAddress: function (coin, userkey, orderId) {
        return build_multisig_address(coin, userkey, orderId)
    },
    // spend it
...
const build_multisig_address = async function (coin, userkey, orderId) {
    let tag = TAG + ' | build_multisig_address | '
    try {
        if (coin === 60) {
            // NOTE this is NOT create multi-sig contract!
            // This is creating forwarder for an order!
            let address = await build_forwarder(orderId, coin, userkey)
            return address
        } else {
            let address = build_ms_address(orderId, coin, userkey)
            return address
        }
    } catch (e) {
        log.error('ERROR: ', e)
...
const build_raw_transaction_multi = async function (origin, destinations, fee) {
    ...
    try {
        ...
        if (coin === 'ETH') {
            ...
            if (destinations[0][outputOne] > dustOutput[normalize_coin(coin)]) outputs[outputOne] = destinations[0][outputOne]
            if (destinations[1][outputTwo] > dustOutput[normalize_coin(coin)]) outputs[outputTwo] = destinations[1][outputTwo]
...
const get_default_fee = function (coin) {
    let fee
    //if (coin === 1 || coin === 0 || coin === 'BTC') fee = 0.0016704
    if (coin === 1 || coin === 0 || coin === 'BTC') fee = 0.0005
    if (coin === 2.1 || coin === 2 || coin === 'LTC') fee = 0.0016704
    return fee
}

const build_ms_address = async function (orderId, coin, userkey) {
    let tag = TAG + ' | build_ms_address | '
    try {
        let cKey1, cKey2, cKey3
        ...
        // coin = normalize_coin(coin)
        log.debug(tag, 'coin: ', coin)
        newAddress = await daemons[normalize_coin(coin)].getNewAddress()
        newAddressInfo = await daemons[normalize_coin(coin)].validateAddress(newAddress)
        ...
        log.debug(tag, 'newAddressInfo: ', newAddressInfo)
        cKey1 = newAddressInfo.pubkey

        // cKey2 = deriveChildPubKeyForOrder(oracleKey, coin, account, index)
        let order = { coin, userkey, orderId, arbiterPubKey: cKey1 }
        log.debug(tag," Asking oracle for pubkey! ")
        cKey2 = await oracle.getNewPubkeyForOrder(order)
        ...
        address = await (daemons[normalize_coin(coin)].addMultiSigAddress(2, pubKeys))
        ...
        addressInfo = await (daemons[normalize_coin(coin)].validateAddress(address))

        addressInfo.orderId = orderId
        // save address info
        addressInfo.addresses = addressInfo.addresses.toString()
        log.debug(tag, 'addressInfo: ', addressInfo)
        await (redis.hmset(address, addressInfo))

        // watch
        log.debug(tag, 'coin: ', coin)
        let success = await watchNewAddress(coin, address)
...
const build_forwarder = async function (orderId, coin, userkey) {
    ...
    try {
        ...
        let address = await eth.getForwarder(payload)
        ...
        let order = { coin, userkey, orderId, depositAddress: address }
        oracle.getNewPubkeyForOrder(order)
